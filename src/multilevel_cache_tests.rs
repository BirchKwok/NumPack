use super::lazy_array::{
    MultiLevelCache, CachePolicy, LRUCache, AdaptiveCache, CompressedCache,
    CacheItemMetadata, MultiLevelCacheReport, CacheConsistencyReport,
    HotSpotDetector, IntelligentEvictionManager, EvictionStrategy, HotSpotStats
};
...
fn test_eviction_strategy_performance() {
    ...
    // 验证性能：应该在合理时间内完成
    assert!(duration.as_millis() < 1000, "Eviction selection should be fast");
...
fn test_hotspot_eviction_integration() {
    ...
    // 应该更多地选择冷数据进行淘汰
    assert!(cold_targets_count >= hot_targets_count, 
