...
use super::{NumPackSIMD, SIMDError, DataType};
...
impl NumPackSIMD {
    ...
    /// AVX2行拷贝实现 - 平衡性能和兼容性
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    pub fn avx2_copy_rows(&self, src: &[u8], dst: &mut [u8], indices: &[usize], row_size: usize) -> Result<(), SIMDError> {
    ...
    /// SSE2行拷贝实现 - 基础SIMD支持
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    pub fn sse2_copy_rows(&self, src: &[u8], dst: &mut [u8], indices: &[usize], row_size: usize) -> Result<(), SIMDError> {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    pub fn simd_int_to_float_convert(&self, src: &[u8], dst: &mut [u8], src_dtype: DataType, dst_dtype: DataType) -> Result<(), SIMDError> {
        if !self.capabilities.avx2 {
    ...
    fn avx2_i32_to_f32(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        let src_i32 = unsafe { 
            std::slice::from_raw_parts(src.as_ptr() as *const i32, src.len() / 4) 
        };
        let dst_f32 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut f32, dst.len() / 4) 
        };
    ...
    fn avx2_i64_to_f64(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        let src_i64 = unsafe { 
            std::slice::from_raw_parts(src.as_ptr() as *const i64, src.len() / 8) 
        };
        let dst_f64 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut f64, dst.len() / 8) 
        };
    ...
    fn avx2_u32_to_f32(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        ...
        unsafe {
            ...
            for i in 0..chunks {
                ...
                // 分别处理高位和低位
                let low_part = _mm256_cvtepi32_ps(_mm256_and_si256(uint_data, _mm256_set1_epi32(0x0000FFFF)));
                let high_part = _mm256_cvtepi32_ps(_mm256_srli_epi32(uint_data, 16));
                
                // 组合结果
                let result = _mm256_add_ps(low_part, _mm256_mul_ps(high_part, _mm256_set1_ps(65536.0f32)));
    ...
    fn avx2_i16_to_f32(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        let src_i16 = unsafe { 
            std::slice::from_raw_parts(src.as_ptr() as *const i16, src.len() / 2) 
        };
        let dst_f32 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut f32, dst.len() / 4) 
        };
    ...
    fn avx2_i8_to_f32(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        let src_i8 = unsafe { 
            std::slice::from_raw_parts(src.as_ptr() as *const i8, src.len()) 
        };
        let dst_f32 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut f32, dst.len() / 4) 
        };
    ...
    /// SIMD浮点到整数转换
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    pub fn simd_float_to_int_convert(&self, src: &[u8], dst: &mut [u8], src_dtype: DataType, dst_dtype: DataType) -> Result<(), SIMDError> {
        if !self.capabilities.avx2 {
            return self.scalar_convert(src, dst, src_dtype, dst_dtype);
        }
    ...
    fn avx2_f32_to_i32(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        let src_f32 = unsafe { 
            std::slice::from_raw_parts(src.as_ptr() as *const f32, src.len() / 4) 
        };
        let dst_i32 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut i32, dst.len() / 4) 
        };
    ...
    fn avx2_f64_to_i64(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        let src_f64 = unsafe { 
            std::slice::from_raw_parts(src.as_ptr() as *const f64, src.len() / 8) 
        };
        let dst_i64 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut i64, dst.len() / 8) 
        };
    ...
    fn avx2_f32_to_i16(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        ...
        let dst_i16 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut i16, dst.len() / 2) 
        };
        ...
        unsafe {
            ...
            for i in 0..chunks {
                ...
                // 提取并存储
                _mm_storeu_si128(dst_i16.as_mut_ptr().add(offset) as *mut __m128i, 
    ...
    fn avx2_f32_to_i8(&self, src: &[u8], dst: &mut [u8]) -> Result<(), SIMDError> {
        ...
        let dst_i8 = unsafe { 
            std::slice::from_raw_parts_mut(dst.as_mut_ptr() as *mut i8, dst.len()) 
        };
        ...
        unsafe {
            ...
            for i in 0..chunks {
                ...
                let int32_data = _mm256_cvttps_epi32(float_data);
                let int16_data = _mm256_packs_epi32(int32_data, int32_data);
                let int8_data = _mm_packs_epi16(_mm256_extracti128_si256(int16_data, 0), 
                                              _mm256_extracti128_si256(int16_data, 0));
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    pub fn simd_widening_convert(&self, src: &[u8], dst: &mut [u8], src_dtype: DataType, dst_dtype: DataType) -> Result<(), SIMDError> {
        // 实现类型扩展转换的SIMD版本
        // 这里为了简化，先使用标量版本
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    pub fn simd_narrowing_convert(&self, src: &[u8], dst: &mut [u8], src_dtype: DataType, dst_dtype: DataType) -> Result<(), SIMDError> {
        // 实现类型收窄转换的SIMD版本
        // 这里为了简化，先使用标量版本
        self.scalar_convert(src, dst, src_dtype, dst_dtype)
    }
