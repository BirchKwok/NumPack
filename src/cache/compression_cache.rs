...
impl AdvancedCompressionCache {
    ...
    pub fn get(&mut self, key: usize) -> Option<Vec<u8>> {
        if self.compressed_data.contains_key(&key) {
            // 获取解压缩所需的信息
            let (data, algorithm, original_size) =
                if let Some(item) = self.compressed_data.get(&key) {
    ...
    pub fn put(&mut self, key: usize, data: Vec<u8>) -> Vec<(usize, Vec<u8>)> {
        ...
        // 执行压缩
        let start_time = Instant::now();
        let (compressed_data, compressed_size) = if original_size >= self.min_compression_size
        ...
        // 记录性能统计
        self.stats
            .record_compression(algorithm, original_size, compressed_size, compression_time);
        self.record_algorithm_performance(
            algorithm,
            original_size,
            compressed_size,
            compression_time,
        ...
        while self.current_size + compressed_size > self.max_size
    ...
    pub fn remove(&mut self, key: usize) -> Option<Vec<u8>> {
        if let (Some(item), Some(_meta)) = (
            self.compressed_data.remove(&key),
    ...
    fn select_compression_algorithm(&self, data: &[u8]) -> CompressionAlgorithm {
        match &self.compression_strategy {
            ...
            CompressionStrategy::Adaptive => self.adaptive_algorithm_selection(data),
    ...
    fn adaptive_algorithm_selection(&self, data: &[u8]) -> CompressionAlgorithm {
        ...
        for (&algorithm, performance) in &self.algorithm_performance {
            if performance.total_compressions > 10 {
                // 需要足够的样本
                // 综合评分：考虑压缩比和速度
    ...
    /// 记录算法性能
    fn record_algorithm_performance(
        &mut self,
        algorithm: CompressionAlgorithm,
        original_size: usize,
        compressed_size: usize,
        ...
        if performance.total_original_bytes > 0 {
            performance.average_ratio =
                performance.total_compressed_bytes as f64 / performance.total_original_bytes as f64;
        }
    ...
    pub fn optimize_compression_strategy(&mut self) {
        ...
        if self.recent_performance_samples.len() >= 100 {
            ...
            for sample in &self.recent_performance_samples {
                let ratio_score =
                    1.0 - (sample.compressed_size as f64 / sample.original_size as f64);
                let speed_score = if sample.compression_time.as_secs_f64() > 0.0 {
                    sample.original_size as f64
                        / sample.compression_time.as_secs_f64()
                        / 1_000_000.0
                } else {
                    0.0
                };
            ...
            // 找到最佳算法
            let best_algorithm = algorithm_scores
                .iter()
