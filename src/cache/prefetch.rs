...
impl MultiLevelPrefetchCache {
    ...
    fn evict_from_l1(&mut self) {
        // 找到访问次数最少的项
        if let Some((&least_used_key, _)) = self.l1_access_count.iter().min_by_key(|(_, &count)| count) {
    ...
    fn evict_from_l2(&mut self) {
        // 找到访问次数最少的项
        if let Some((&least_used_key, _)) = self.l2_access_count.iter().min_by_key(|(_, &count)| count) {
...
impl PrefetchManager {
    ...
    fn execute_intelligent_prefetch(&mut self, indices: &[usize]) {
        ...
        for &idx in indices {
            ...
            // 检查内存带宽限制
            if self.bytes_prefetched_this_second + prefetched_bytes + estimate_item_size > self.memory_bandwidth_limit {
                break;
            }
