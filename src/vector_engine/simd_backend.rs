...
// 导入 SimSIMD traits
use simsimd::{SpatialSimilarity, ProbabilitySimilarity, BinarySimilarity};
...
impl SimdBackend {
    ...
    pub fn compute_f64(&self, a: &[f64], b: &[f64], metric: MetricType) -> Result<f64> {
        ...
        match metric {
            ...
            MetricType::Cosine => {
                ...
                // 我们需要转换为余弦相似度
                let distance = simsimd::SpatialSimilarity::cosine(a, b).expect("SimSIMD cosine failed");
            ...
            MetricType::L2Squared => {
                Ok(simsimd::SpatialSimilarity::sqeuclidean(a, b).expect("SimSIMD sqeuclidean failed"))
            }
            MetricType::KL => {
                // KL 散度 (Kullback-Leibler Divergence)
                Ok(simsimd::ProbabilitySimilarity::kullbackleibler(a, b).expect("SimSIMD KL failed"))
            }
            MetricType::JS => {
            ...
            MetricType::Hamming | MetricType::Jaccard => {
                Err(SimdError::UnsupportedMetric {
                    metric,
    ...
    pub fn compute_f32(&self, a: &[f32], b: &[f32], metric: MetricType) -> Result<f32> {
        ...
        match metric {
            MetricType::DotProduct | MetricType::InnerProduct => {
                let result: f64 = simsimd::SpatialSimilarity::dot(a, b).expect("SimSIMD dot failed");
                Ok(result as f32)
            }
            MetricType::Cosine => {
                // SimSIMD 返回的是余弦距离 (1 - cosine_similarity)
                // 我们需要转换为余弦相似度
                let distance: f64 = simsimd::SpatialSimilarity::cosine(a, b).expect("SimSIMD cosine failed");
            ...
            MetricType::L2Squared => {
                let result: f64 = simsimd::SpatialSimilarity::sqeuclidean(a, b).expect("SimSIMD sqeuclidean failed");
                Ok(result as f32)
            }
            MetricType::KL => {
                // KL 散度
                let result: f64 = simsimd::ProbabilitySimilarity::kullbackleibler(a, b).expect("SimSIMD KL failed");
                Ok(result as f32)
            }
            MetricType::JS => {
                // JS 散度
                let result: f64 = simsimd::ProbabilitySimilarity::jensenshannon(a, b).expect("SimSIMD JS failed");
                Ok(result as f32)
            }
            MetricType::Hamming | MetricType::Jaccard => {
                Err(SimdError::UnsupportedMetric {
    ...
    pub fn compute_i8(&self, a: &[i8], b: &[i8], metric: MetricType) -> Result<f64> {
        ...
        match metric {
            ...
            MetricType::Cosine => {
                let distance = simsimd::SpatialSimilarity::cosine(a, b).expect("SimSIMD i8 cosine failed");
                Ok(1.0 - distance)
            }
            MetricType::L2Distance => {
                let sq = simsimd::SpatialSimilarity::sqeuclidean(a, b).expect("SimSIMD i8 sqeuclidean failed");
                Ok(sq.sqrt())
            }
            MetricType::L2Squared => {
                Ok(simsimd::SpatialSimilarity::sqeuclidean(a, b).expect("SimSIMD i8 sqeuclidean failed"))
            }
