...
impl SmartIndexRouter {
    ...
    pub fn route_fancy_index(&self, 
        ...
        let pattern = self.profiler.analyze_access_pattern(0, indices.len() * array.itemsize);
    ...
    pub fn route_boolean_index(&self, 
        ...
        let selected_count = mask.iter().filter(|&&x| x).count();
        let pattern = self.profiler.analyze_access_pattern(0, selected_count * array.itemsize);
