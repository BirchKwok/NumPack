...
def test_large_data():
    ...
    try:
        ...
        # Test NumPack replace operation
        logger.info("Test NumPack large data replace...")
        replace_data = {
            'array1': np.random.rand(size, 10).astype(np.float32)
        }
        start_time = time.time()
        ...
        logger.info(f"NumPack replace time: {replace_time:.2f} seconds")
        
        # NumPy npz does not support in-place replacement, so the entire file needs to be saved again
        logger.info("Test NumPy npz large data replace...")
        ...
        logger.info(f"NumPy npz replace time: {npz_replace_time:.2f} seconds")
        logger.info(f"Replace performance comparison (npz): NumPack/NumPy = {replace_time/npz_replace_time:.2f}x")
        
        # NumPy npy does not support in-place replacement, so the entire file needs to be saved again
        logger.info("Test NumPy npy large data replace...")
        start_time = time.time()
        np.save('test_large_array1.npy', replace_data['array1'])
        npy_replace_time = time.time() - start_time
        logger.info(f"NumPy npy replace time: {npy_replace_time:.2f} seconds")
        logger.info(f"Replace performance comparison (npy): NumPack/NumPy = {replace_time/npy_replace_time:.2f}x")
