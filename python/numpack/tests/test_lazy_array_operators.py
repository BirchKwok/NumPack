...
def test_lazy_array_arithmetic_operators():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_arithmetic")
        pack = npk.pack(array_path, [original_data], ["test_array"])

        # 加载为 LazyArray
        lazy_array = npk.load(array_path, lazy=True)
...
def test_lazy_array_comparison_operators():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_comparison")
        pack = npk.pack(array_path, [original_data], ["test_array"])
...
def test_lazy_array_unary_operators():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_unary")
        pack = npk.pack(array_path, [original_data], ["test_array"])
...
def test_lazy_array_bitwise_operators():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_bitwise")
        pack = npk.pack(array_path, [original_data], ["test_array"])
...
def test_lazy_array_inplace_operators():
    """测试原地操作符（应该抛出 NotImplementedError）"""
    print("测试原地操作符...")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_inplace")
        pack = npk.pack(array_path, [original_data], ["test_array"])
        ...
        try:
            lazy_array += 2.5
            assert False, "原地加法应该抛出 NotImplementedError"
        except NotImplementedError as e:
            assert "In-place operations are not supported" in str(e)
            print("✓ 原地加法操作符正确抛出错误")

        # 测试原地乘法（应该失败）
        try:
            lazy_array *= 2.5
            assert False, "原地乘法应该抛出 NotImplementedError"
        except NotImplementedError as e:
            assert "In-place operations are not supported" in str(e)
            print("✓ 原地乘法操作符正确抛出错误")
...
def test_lazy_array_bitwise_type_checking():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_bitwise_type_check")
        pack = npk.pack(array_path, [original_data], ["test_array"])
...
def test_lazy_array_complex_operations():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 保存为 LazyArray
        array_path = os.path.join(tmpdir, "test_complex")
        pack = npk.pack(array_path, [original_data], ["test_array"])
